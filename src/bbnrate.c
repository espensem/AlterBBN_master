 ...
 * (-err)*/
{
    ...
    if((!paramrelic.wimp_added)&&((paramrelic.xinu1==0.)||(Tnu==0.)))
        /* No neutrino degeneracy */
    {
        ...
        if(z<5.10998997931)
        {
            ...
            r[1]*=exp(-2.8602*z)/paramrelic.life_neutron; /* p->n */
        }
...
void rate_all(int err, double f[], double T9, struct relicparam paramrelic)
 ...
 * random gaussian error; err<0: error only for process number (-err) */
{
	...
	if((err!=0)&&(err!=-10000)) for(ie=12;ie<=NNUCREAC;ie++)
	{
		ferrhigh[ie]=9.;
		ferrlow[ie]=-0.9;
		ferr[ie]=ferrhigh[ie];
...
/* C13 + d -> p + C14 */    // Iocco et al 2007
    ...
    if((err!=0)&&(err!=-10000))
    {
        ferrlow[100]=-0.9;
        ferrhigh[100]=9.0;
        ferr[100]=ferrlow[100];
    }


/* ############################################################# */

	for(ie=12;ie<=NNUCREAC;ie++) f[ie]=fabs(f[ie]);
	...
	if(err>0)
	{
		...
		{
			if((fabs(1.+ferrhigh[ie])<1.)||(fabs(1.+ferrhigh[ie])>10.)) ferrhigh[ie]=9.;
			if((fabs(1.+ferrlow[ie])>1.)||(fabs(1.+ferrlow[ie])<0.1)) ferrlow[ie]=-0.9;
			if(err==1) f[ie]*=fabs(1.+ferrhigh[ie]);
			if(err==2) f[ie]*=fabs(1.+ferrlow[ie]);
			
			if(err>100000) f[ie]*=fabs(1.+ferr[ie]*rand_gauss());
		}
	}
	else if(err<=-12&&err>=-NNUCREAC) f[-err]*=fabs(1.+ferr[-err]);
